...
header = oldheader + ['percolator svm-score', 'PSM q-value', 'peptide q-value', 'missed_cleavage']
        ...
        for specidr in mzidplus.mzid_spec_result_generator(mzidfn, mzns):
            for specidi in specidr.findall('{%s}SpectrumIdentificationItem' % mzns['xmlns']):
                ...
                try:
                    ...
                    continue
                outpsm.update({'percolator svm-score': percopsm['svm'], 'PSM q-value': percopsm['qval'], 'peptide q-value': percopsm['pepqval'], 'missed_cleavage': count_missed_cleavage(outpsm['Peptide'])})
                if percopsm['decoy']:
                    dfp.write('\n')
